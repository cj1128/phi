...
func TestTree(t *testing.T) {
	...
	hIndex := newStub()
	hFavicon := newStub()
	hArticleList := newStub()
	hArticleNear := newStub()
	hArticleShow := newStub()
	hArticleShowRelated := newStub()
	hArticleShowOpts := newStub()
	hArticleSlug := newStub()
	hArticleByUser := newStub()
	hUserList := newStub()
	hUserShow := newStub()
	hAdminCatchall := newStub()
	hAdminAppShow := newStub()
	hAdminAppShowCatchall := newStub()
	hUserProfile := newStub()
	hUserSuper := newStub()
	hUserAll := newStub()
	hHubView1 := newStub()
	hHubView2 := newStub()
